...
impl Job {
	...
	pub async fn run(&mut self, config: &Config, job_id: usize) -> Result<()> {
		...
		let parts = shell_words::split(&self.command)
			.map_err(|e| CronrError::JobExecutionError(format!("Failed to parse command: {}", e)))?;
		...
		let child = match command.spawn() {
			Ok(child) => child,
			Err(e) => {
				return Err(CronrError::JobExecutionError(format!("Failed to spawn command: {}", e)));
			}
		};
		...
		let output = match child.wait_with_output() {
			Ok(output) => output,
			Err(e) => {
				return Err(CronrError::JobExecutionError(format!("Failed to wait for command: {}", e)));
			}
		};
	...
	pub async fn execute_with_schedule(
		...
		let mut next_run_time = match job.next_run() {
			...
			None => {
				...
				match job.next_run() {
					Some(time) => time,
					None => return Err(CronrError::JobExecutionError("Could not calculate next run time".into())),
				}
			}
		};
		...
		loop {
			...
			if next_run_time > now {
				...
				log::debug!("Job {} sleeping for {} seconds", id, sleep_duration.as_secs());
			...
			if now >= next_run_time {
				...
				next_run_time = match job.next_run() {
					...
					None => {
						...
						return Err(CronrError::JobExecutionError("Could not calculate next run time".into()));
					}
				};
