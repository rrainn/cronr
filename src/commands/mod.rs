...
pub fn run(cli: Cli) -> Result<()> {
	// Handle commands
	match cli.command {
		Some(Commands::Create { command, cron_expression }) => create_job(command, cron_expression),
...
fn create_job(command: String, cron_expression: String) -> Result<()> {
	// Create the runtime
	let rt = Runtime::new()
		.map_err(|e| CronrError::InitializationError(format!("Failed to create async runtime: {}", e)))?;
	...
	rt.block_on(async {
		...
		// Add the job
		let id = job_manager.add_job(command.clone(), cron_expression.clone()).await?;
