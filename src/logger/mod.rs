...
impl Logger {
	...
	fn write_log(&self, path: &PathBuf, data: &[u8]) -> Result<()> {
		...
		self.rotation.check_rotation(path)
...
mod tests {
	...
	fn test_log_rotation() {
		...
		// Create a test file
		{
			...
			file.write_all(b"test data that is larger than 100 bytes...").unwrap();

			// Add more data to exceed max_size
			file.write_all(b"more test data that exceeds the 100 byte limit for this test").unwrap();
		}
