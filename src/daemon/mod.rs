...
impl Daemon {
	...
	pub fn start(&self) -> Result<()> {
		...
		if self.is_running() {
			return Err(CronrError::DaemonStartFailed("Daemon is already running".into()));
		}
		...
		let daemonize = Daemonize::new()
			...
			.stdout(stdout.try_clone().map_err(|e| path_error_to_config_error(&stdout_file, e))?)
			.stderr(stdout);
		...
		match daemonize.start() {
			Ok(_) => {
				...
				let exe = std::env::current_exe()
					.map_err(|e| CronrError::DaemonStartFailed(format!("Failed to get executable path: {}", e)))?;

				let status = Command::new(exe)
					.arg("daemon-internal")
					.status()
					.map_err(|e| CronrError::DaemonStartFailed(format!("Failed to start daemon process: {}", e)))?;

				// This should not be reached in the daemon process
				if !status.success() {
					return Err(CronrError::DaemonStartFailed(format!("Daemon process exited with status {}", status.code().unwrap_or(-1))));
				}
			...
			Err(e) => {
				...
				return Err(CronrError::DaemonStartFailed(format!("Failed to daemonize: {}", e)));
			}
		}
	}
	...
	pub fn stop(&self) -> Result<()> {
		...
		if (!self.is_running()) {
			return Err(CronrError::DaemonStartFailed("Daemon is not running".into()));
		}

		// Get the PID
		let pid_file = self.pid_file();
		let pid_str = fs::read_to_string(&pid_file)
			.map_err(|e| path_error_to_config_error(&pid_file, e))?;
		...
		#[cfg(target_os = "linux")]
		{
			...
			kill(Pid::from_raw(pid as i32), Signal::SIGTERM)
				.map_err(|e| CronrError::DaemonStartFailed(format!("Failed to kill daemon: {}", e)))?;
		}
		...
		#[cfg(target_os = "windows")]
		{
			...
			if !status.success() {
				return Err(CronrError::DaemonStartFailed(format!("Failed to kill daemon, taskkill returned {}", status.code().unwrap_or(-1))));
			}
		}
	...
	pub fn register_for_startup(&self) -> Result<()> {
		...
		#[cfg(target_os = "windows")]
		{
			...
			let exe = std::env::current_exe()
				.map_err(|e| CronrError::InitializationError(format!("Failed to get executable path: {}", e)))?;
			...
			let status = Command::new("schtasks")
				.args(&[
					...
					"/sc", "onstart",
					"/ru", "SYSTEM",
					"/tr", &format!("\"{}\" start", exe.display()),
					"/f"
				])
				.status()
				.map_err(|e| CronrError::InitializationError(format!("Failed to register with Task Scheduler: {}", e)))?;
...
impl DaemonRunner {
	...
	pub async fn run(&mut self) -> Result<()> {
		...
		loop {
			...
			for (id, job) in &jobs {
				log::debug!("Job {} details: command={}, enabled={}, next_run={:?}, last_executed={:?}",
					id, job.command(), job.enabled, job.next_run(), job.last_executed);
			}
	...
	pub async fn stop_job(&mut self, id: usize) -> Result<()> {
		...
		stop_tx.send(true)
			.map_err(|_| CronrError::CommandExecutionFailed(format!("Failed to send stop signal to job {}", id)))?;

		// Wait for the job to stop
		handle.await
			.map_err(|e| CronrError::CommandExecutionFailed(format!("Failed to join job task: {}", e)))??;
	...
	async fn wait_for_signal(&self) -> Result<()> {
		...
		#[cfg(unix)]
		{
			...
			let mut sigterm = signal(SignalKind::terminate())
				.map_err(|e| CronrError::InitializationError(format!("Failed to set up signal handler: {}", e)))?;
